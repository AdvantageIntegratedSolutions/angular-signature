...
angular.module('signature').constant('SignaturePad', require('signature_pad'));

angular.module('signature').directive('signaturePad', ['$interval', '$timeout', '$window', 'SignaturePad',
  function ($interval, $timeout, $window, SignaturePad) {
    'use strict';
    ...
    return {
      restrict: 'EA',
      replace: true,
      template: '<div class="signature" style="width: 100%; max-width:{{width}}px; height: 100%; max-height:{{height}}px;"><canvas style="display: block; margin: 0 auto;" ng-mouseleave="onMouseup()" ng-mouseup="onMouseup()" ng-mousedown="notifyDrawing({ drawing: true })"></canvas></div>',
      scope: {
        accept: '=?',
        clear: '=?',
        disabled: '=?',
